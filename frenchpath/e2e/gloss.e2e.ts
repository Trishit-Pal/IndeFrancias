...
test('tap French word in exercise shows gloss popover', async ({ page }) => {
	await gotoHome(page);
	await page.getByTestId('unit-card').first().click();
	await page.getByTestId('start-lesson').click();
	await reachExercisePhase(page);

	const glossWord = page.getByTestId('gloss-word').first();
	await expect(glossWord).toBeVisible();
	await glossWord.click();
	await expect(page.getByTestId('gloss-popover')).toBeVisible();
});

test('bridge body supports tap-to-gloss', async ({ page }) => {
	await gotoHome(page);
	await page.getByTestId('unit-card').first().click();
	await expect(page.getByTestId('gloss-markdown')).toBeVisible({ timeout: 10_000 });
	const glossWord = page.getByTestId('gloss-markdown').getByTestId('gloss-word').first();
	await glossWord.click();
	await expect(page.getByTestId('gloss-popover')).toBeVisible();
});
