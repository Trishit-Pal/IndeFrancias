...
| CAP-TTS | TTS + record/compare + shadowing | shipped | `frenchpath/src/lib/audio/tts.ts`, `frenchpath/src/lib/platform/tts.ts`, `frenchpath/src/lib/lesson/ShadowingPlayer.svelte` | `tts-section`, `tts-voice-select`, `tts-speed-slider`, `shadow-toggle`, `shadow-transcript`; `e2e/tts.e2e.ts` | mobile-architecture |
...
**Test baseline:** 256 unit + 42 e2e green (the authoritative count; supersedes any differing
